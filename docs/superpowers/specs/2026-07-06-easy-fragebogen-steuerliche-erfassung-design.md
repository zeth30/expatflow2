...
- **Privacy as a headline selling point (user requirement):** this form contains highly sensitive data (Steuer-ID, IBAN, income estimates). The landing page, wizard, and answer sheet all state prominently and in simple English: *all answers stay in your browser (localStorage) — nothing is sent to or stored on our servers; the only server-side data is the Stripe payment and, optionally, a first name + email for the reminder.* The privacy promise also gets its own trust block on the landing page (same pattern as Anmeldung, but more prominent given IBAN/Steuer-ID sensitivity) and a line in the Terms/Datenschutz section.
